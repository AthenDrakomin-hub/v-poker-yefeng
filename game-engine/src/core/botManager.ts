...
export class BotManager {
  ...
  private executeBotAction(seatIndex: number, userId: string): void {
    ...
    let action: "fold" | "check" | "call" | "raise" | "all_in" | "bet";
    let amount: number | undefined;

    const roll = Math.random();

    if (callAmount <= 0) {
      // 没有人下注，我可以 check 或 bet
      if (roll < 0.6) {
        action = "check";
      } else if (roll < 0.85) {
        action = "bet";
      ...
      const callRatio = callAmount / myChips;
    ...
    console.log(`[BotManager] Bot ${userId} at seat ${seatIndex} acts: ${action} ${amount || ""}`);
