 ...
 *
 * v3 新增：
 *  - TurnTimer 回合动作倒计时（默认 30s，超时自动 check/fold）
 *  - RoundEventLog 事件日志（局末落库 game_replays）
 *  - startRound 改为 async，等待盲注扣款完成
 */
...
export class GameStateMachine {
  ...
  /**
   * 开始一局（async：等待盲注扣款完成）
   * 修复 v2 bug：原实现调用 async processBlindBets 未 await，盲注扣款可能失败
   */
  public async startRound(): Promise<boolean> {
    ...
    // 发牌后处理小盲/大盲的下注（自动 bet）—— async，必须 await
  ...
  public handleAction(action: GameAction): { success: boolean; error?: string } {
    ...
    // 阶段推进后，如果还在 BETTING，启动下一个行动玩家的倒计时
    this.maybeStartTurnTimer();
  ...
  /** 如果当前在 BETTING 阶段且有行动玩家，启动倒计时 */
  private maybeStartTurnTimer(): void {
    if (this.roundState.phase !== "BETTING") return;
    ...
    // 轮到 bot：交给 BotManager 决策，不启动人类倒计时
