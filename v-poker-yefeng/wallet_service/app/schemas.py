...
class CreateRoomRequest(BaseModel):
    ...
    room_level: str = Field(default="low", description="房间等级: low/mid/high")
    blind_small: int = Field(default=5, gt=0, description="小盲注")
    blind_big: int = Field(default=10, gt=0, description="大盲注")
    ante: int = Field(default=0, ge=0, description="前注")
    variant: str = Field(default="normal", description="规则变体: normal/aof")
    mode: str = Field(default="cash", description="游戏模式")
    total_rounds: int = Field(default=10, ge=1, le=50, description="总局数")
    base_score: int = Field(default=10, gt=0, description="底分（兼容旧字段）")
    action_time: int = Field(default=20, ge=10, le=60, description="行动时间(秒)")
    ...
    min_players: int = Field(default=2, ge=2, le=9, description="最低开局人数")
    max_players: int = Field(default=6, ge=2, le=9, description="最大人数")
    big_blind: Optional[int] = Field(default=None, gt=0, description="大盲注（兼容旧字段）")
    ...
    config: Optional[dict] = Field(default=None, description="游戏特殊配置")
