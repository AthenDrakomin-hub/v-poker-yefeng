/**
 * 牛牛规则插件 (支持模式: qiang_zhuang 抢庄牛牛, tong_bi 通比牛牛)
 */
...
export class NiuNiuPlugin implements GamePlugin {
  ...
  handleAction(state: PluginRoundState, action: GameAction): { success: boolean; error?: string } {
    ...
    return { success: false, error: `Unsupported action ${action.action_type} for Niu Niu.` };
  }
  ...
  compareHands(state: PluginRoundState): CompareResult {
    const activeSeats = state.seats.filter((s) => s.status === "playing" && s.cards.length === 5);
    ...
    rankings.sort((a, b) => {
      ...
      return b.evaluation.rank_level - a.evaluation.rank_level;
    });
  ...
  calculateNetScores(state: PluginRoundState): PlayerNetResult[] {
    ...
    if (state.room.mode === "qiang_zhuang") {
      ...
      // 通比牛牛模式：全场横向比牌，牌型第一名赢取其余所有闲家的输注 (或每两两比对)
      const comp = this.compareHands(state);
      const winnerId = comp.winner_user_ids[0];
      const winnerEval = comp.rankings[0].evaluation;
      const winMult = winnerEval.multiplier;
